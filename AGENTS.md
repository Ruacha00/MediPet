...
派发、接取或更新重构任务时，阅读 `docs/internal/rebuild/README.md`，按其中的 spec、issue 依赖与共享文件约定安排并行工作。

当前正在按计划实施。恢复或接续执行时先读 `docs/internal/rebuild/EXECUTION.md`，核对活动任务和验收记录；旧目录不作为新实现模板。
