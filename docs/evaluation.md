...
最新[复合交付与知识回答专项](../evaluation/reports/delivery-recall-20260918/notes.md)记录产品修复及前后对照；[回答评测说明](../evaluation/answer-quality.md)定义30题双组的正确性、证据支持、需求覆盖、无依据断言和资料不足处理。其生成器只消费实际检索证据，与产品/chat业务链分开，不能将两类通过率混用。

该专项实测复合交付11/20→20/20，完整检索Recall@3为51.25%→97.5%，检索证据条件下回答综合通过16/30→28/30。新版回答仍有1条Judge无效和1条跨文档事实缺失，均计入固定分母；报告保留候选状态。具体题集范围、评分阈值和失败原因见专项，不将这些小规模回归结果表述为线上业务准确率。

修复前[效果扩测](../evaluation/reports/business-expanded-20260918/notes.md)覆盖80个业务任务、40条检索问题及186条意图回归：预约完成40/40、上下文衔接19/20、复合交付11/20；检索Recall@3由26.25%提升到50%，意图准确率99.46%、Macro-F1 0.995。扩测使用`evaluation.expanded_metrics`，独立检索/意图脚本位于`tests/benchmarks/`；完整失败和口径见原报告，不能用后续修复覆盖这些结果。

2026-09-18新增[业务指标专项](../evaluation/reports/business-20260918/notes.md)：在main `bf4f353`上完整运行既有66/15/12输入，再加入12组预约和复合表达，保存37/39业务状态组与52/54质量轮次结果。另用真实Redis完成50组重复确认、30组末号竞争检查。按预约、记忆、复合交付分母分别计算，不把混合通过率直接写成业务成功率；失败与候选状态保留。

专项入口为`python -m evaluation.business_metrics --output 新目录 --execute`，不带`--execute`只准备输入。输入、部署源码、接受基线指纹和逐组报告均保存；汇总器另验证缺失分母、跨轮拼接和调用失败的计分规则。并发事务脚本为`tests/benchmarks/business_transactions.py`，执行方法与限制见专项报告。
