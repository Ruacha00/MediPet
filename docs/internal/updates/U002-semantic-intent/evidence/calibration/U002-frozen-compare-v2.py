    ...
    for name,path in (("holdout",Path("evaluation/cases/semantic_intent/holdout.json")),("regression",Path("evaluation/cases/intents.json"))):
        ...
        try:
            for case in ds["cases"]:
                ...
                if captured["bypassed_emergency"]:
                    assert old.detect_emergency(case["message"])
                    oldrows.append({**e.case_fields(case),"prediction":"emergency","source_scores":{"llm":0.0,"embedding":0.0,"pattern":1.0},"fixed_rule_bypass":True})
                    continue
