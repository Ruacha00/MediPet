...
MediPet 是一家虚构医院的门诊就诊助手。为本人或家属选择就诊人后，可以在同一个事项中查号源、准备材料、确认预约、查看记录、获取院内文字指引和取消预约。
...
预约方案不会预占库存，聊天中的“确认”也不执行预约；以卡片按钮确认后的回执为准。人工导诊功能提供联系资料，不向工作人员发送消息。项目不接入真实医院，不提供诊断、处方、支付或实时地图导航。
...
首次构建会下载依赖并缓存 Chroma 客户端的默认向量模型。等待 API 和前端健康后，打开 [MediPet 页面](http://localhost:8088)。页面通过同源 `/api/python` 访问 Python API。
...
Redis、Chroma 和 API 的运行数据使用命名卷。上述停止命令保留卷；初始化只补缺失默认数据。Skills 目录以只读挂载提供给 API，修改宿主机文件后在“知识库 → 已加载能力”点击“重新加载”。
...
容器初次启动、再次启动、四容器重建后预约及历史保留、同源代理和定向重置均已通过[真实验证](docs/internal/rebuild/evidence/runtime.md)。日常停止无需重置数据。需要清空本项目演示记录时，先停止应用写入，再预览目标并执行：
...
该操作删除 `medipet:*` Redis 键及 `medipet_knowledge`、`medipet_episodic`、`medipet_profiles` 三个精确集合，包含事项、预约、历史、已导入资料和记忆。普通启动补回两位预置患者、默认号源与 17 份知识文档。其他前缀的 Redis 数据、名单以外的 Chroma 集合、数据卷和评测报告保留。

## 演示与代码导航

按[固定演示脚本](docs/demo-script.md)体验“孩子查号与材料 → 确认预约 → 恢复历史 → 指引 → 取消”。[面试说明](docs/interview-notes.md)把设计理由、代码入口、证据和取舍放在一起。
...
2026-09-16 最终代码回归为后端 **389 passed、21 skipped**，前端 **32/32** 及构建通过；21 项需要显式启用真实组件，其独立执行证据见[存储验证](docs/internal/rebuild/evidence/storage.md)与[容器验证](docs/internal/rebuild/evidence/runtime.md)。真实模型的页面业务闭环和管理操作见[浏览器记录](docs/internal/rebuild/evidence/browser.md)。测试项通过数不是模型准确率，也不与独立子套结果相加。

评测输入包括 54 条意图、12 组多轮和 12 组边界场景。每次运行生成 candidate，调用失败、Judge 失败、跳过和业务断言分别保留，人工复核后才能接受为基线。完整同步评测会发起多次模型请求；Nginx 仅为 `/api/python/eval/run` 设置 1800 秒等待，其他请求仍为 180 秒。
