...
执行拆分见 [重构工作索引](docs/internal/rebuild/README.md)：8 份 spec、40 个原子 issue，以及直接依赖、可并行批次和共享文件安排。本文继续维护产品范围与完成定义，issue 维护实施状态和验收记录；当前交接点见 [执行检查点](docs/internal/rebuild/EXECUTION.md)。
...
7. 准备阶段已完成实施计划与新工作树清理，用户现已授权实施。各阶段的实际导入、运行和验证状态以 issue 与执行检查点记录为准。
...
本文涉及基准代码的路径与符号，均指上述固定版本中的文件；P1 已将选定代码导入本工作树，必要差异见内部源码对照。旧 MediPet 的图谱与新实现不一致，不能用旧图谱的调用数量判定新代码影响范围。当前工作树索引已重建，后续继续按仓库要求做修改前影响分析、重命名分析和提交前变更分析。
...
阶段验收顺序：P1 → P2 → P3 → P4 → P5 → P6。按本次并行推进要求，P1 基准验收完成后，P2—P5 的独立子任务可以依据 [issue 直接依赖与共享文件安排](docs/internal/rebuild/DEPENDENCIES.md) 并行实施，知识内容、评测数据和前端组件也可在输入契约稳定后提前准备。阶段标签不额外阻塞独立任务，但阶段通过仍须具备相应能力及集成证据，局部假模型或组件验收不等于整阶段完成。后续是否提交、合并或切换默认分支单独按当时授权处理，本计划不自动触发这些操作。
...
以下命令已在对应阶段执行；各层验证结果见内部执行检查点与证据记录，完整后端最终为 389 passed、21 skipped，前端 32 项测试及生产构建通过。真实存储和实际模型的结果独立记录。
...
技术检查已有对应证据；正式基线仍须按 §14.1 人工复核接受。当前报告为 candidate，重构尚未标记完成。

### 产品功能

- [x] R01—R12 均有可运行入口和对应验收证据。
- [x] 医院、医生、号源、费用、地点在工具、知识、页面和评测中一致。
- [x] 页面完成查询、选择、确认、记录查询和取消闭环。
- [x] 未确认不产生预约；重复确认返回同一次执行结果。
- [x] 本人和家属的上下文、事项及预约不串用。
- [x] 历史可刷新恢复、归档恢复，工作记忆压缩不删除完整消息。
- [x] 复合需求能够展示真实主辅协作与各自结果。

### 技术机制与质量

- [x] 三路意图、主辅编排、工具循环、RAG、记忆、Skills、监控和评测均可对应实际代码讲解。
- [x] 成功与失败请求均保留可核对的 trace；结构化卡片来自业务数据。
- [x] 急症演示用例不依赖普通模型调用完成；否定与一般咨询反例不被错误执行为预约或确认。
- [x] 业务测试全部通过，真实存储集成与浏览器关键路径通过。
- [x] 前端构建和 Compose 启动验证通过。
- [x] 实际模型评测结果完整保存；任何未通过项明确列出，未伪造提升数据。
- [x] 代码及注释、功能界面、配置、默认数据、容器、README 和其他对外展示文档完成品牌与旧场景扫描；内部实施计划和源码对照不适用来源名称禁用规则。

### 仓库交付

- [x] 工作仍属于现有 MediPet 仓库，未初始化第二个仓库。
- [x] 新目录 `.git` 关联有效；未覆盖主仓库的既有修改和备份。
- [x] 新版本只保留一套有效启动路径和架构说明。
- [x] 源路径清单和个人运行数据未进入项目提交。
- [x] 按工作树实际代码完成修改影响分析与提交前变更检查；不将旧索引的空结果当作通过。见[最终检查](docs/internal/rebuild/evidence/change-analysis-final.md)，当前提交安排见[执行检查点](docs/internal/rebuild/EXECUTION.md)。
